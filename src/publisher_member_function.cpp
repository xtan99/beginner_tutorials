...
using SERVICE =
    rclcpp::Service<beginner_tutorials::srv::PrintNewString>::SharedPtr;
...
class MinimalPublisher : public rclcpp::Node {
 public:
  ...
  explicit MinimalPublisher(char * talk[]) : Node("minimal_publisher"), m_count_(0) {
    ...
    auto serviceCallbackPtr =
        std::bind(&MinimalPublisher::print_new, this, _1, _2);
    m_service_ =
        create_service<PRINTNEWSTRING>(serviceName, serviceCallbackPtr);

    this->declare_parameter(
        "my_parameter",
        "Hello");  // Declaring the parameter for publisher node which is the
 ...
 private:
   ...
   */
  void print_new(REQUEST request, RESPONSE response) {
    response->new_string = request->s;
    RCLCPP_INFO_STREAM(this->get_logger(),
                       "Incoming request: " << response->new_string);
    RCLCPP_INFO_STREAM(this->get_logger(),
                       "Sending back response:" << response->new_string);
