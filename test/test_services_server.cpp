/**
 * @file test_services_server.cpp
 * @author Tanmay Haldankar (tanmay.n.haldankar99@gmail.com)
 * @brief
 * @version 0.1
 * @date 2022-11-30
 *
 * @copyright MIT License (c)
 *
 */
#include <memory>
#include <utility>

#include "beginner_tutorials/srv/print_new_string.hpp"
#include "rclcpp/rclcpp.hpp"

/**
 * @brief Function to set response according to request for text
 *
 * @param request
 * @param response
 */
void handle_add_two_ints_noreqid(
    const std::shared_ptr<beginner_tutorials::srv::PrintNewString::Request>
        request,
    std::shared_ptr<beginner_tutorials::srv::PrintNewString::Response>
        response) {
  response->new_string = request->s;
}

/**
 * @brief Function to set response according to request for text
 *
 * @param request_header
 * @param request
 * @param response
 */
void handle_add_two_ints_reqid(
    const std::shared_ptr<rmw_request_id_t> request_header,
    const std::shared_ptr<beginner_tutorials::srv::PrintNewString::Request>
        request,
    const std::shared_ptr<beginner_tutorials::srv::PrintNewString::Response>
        response) {
  (void)request_header;
  response->new_string = request->s;
}

/**
 * @brief Class to initialize service server
 *
 */
class DeferedCbServiceWrapper
    : public std::enable_shared_from_this<DeferedCbServiceWrapper> {
 public:
  DeferedCbServiceWrapper() = default;

  void create_service(rclcpp::Node& node) {
    impl_ = node.create_service<beginner_tutorials::srv::PrintNewString>(
        "add_two_ints_defered_cb",
        [this](const std::shared_ptr<rmw_request_id_t> request_header,
               const std::shared_ptr<
                   beginner_tutorials::srv::PrintNewString::Request>
                   request) {
          if (handle_defered_response_thread_.joinable()) {
            throw std::runtime_error{
                "expected the callback to be called only once"};
          }
          auto shared_this = this->shared_from_this();
          handle_defered_response_thread_ = std::thread(
              [me = std::move(shared_this), request = std::move(request),
               request_header = std::move(request_header)]() {
                beginner_tutorials::srv::PrintNewString::Response response;
                response.new_string = request->s;
                me->impl_->send_response(*request_header, response);
              });
        });
  }

  ~DeferedCbServiceWrapper() {
    if (handle_defered_response_thread_.joinable()) {
      handle_defered_response_thread_.join();
    }
  }

 private:
  std::shared_ptr<rclcpp::Service<beginner_tutorials::srv::PrintNewString>>
      impl_;
  std::thread handle_defered_response_thread_;
};

/**
 * @brief Main function
 *
 * @param argc
 * @param argv
 * @return int
 */
int main(int argc, char** argv) {
  rclcpp::init(argc, argv);

  auto node = rclcpp::Node::make_shared("add_two_ints_server");

  auto service_noreqid =
      node->create_service<beginner_tutorials::srv::PrintNewString>(
          "add_two_ints_noreqid", handle_add_two_ints_noreqid);

  auto service_reqid =
      node->create_service<beginner_tutorials::srv::PrintNewString>(
          "add_two_ints_reqid", handle_add_two_ints_reqid);

  auto service_return_req =
      node->create_service<beginner_tutorials::srv::PrintNewString>(
          "add_two_ints_reqid_return_request", handle_add_two_ints_reqid);

  auto defered_cb = std::make_shared<DeferedCbServiceWrapper>();
  defered_cb->create_service(*node);

  rclcpp::TimerBase::SharedPtr timer;
  auto derefed_cb_with_handle =
      node->create_service<beginner_tutorials::srv::PrintNewString>(
          "add_two_ints_defered_cb_with_handle",
          [node, &timer](
              const std::shared_ptr<
                  rclcpp::Service<beginner_tutorials::srv::PrintNewString>>
                  handle,
              const std::shared_ptr<rmw_request_id_t> request_header,
              const std::shared_ptr<
                  beginner_tutorials::srv::PrintNewString::Request>
                  request) {
            // We defer handling the response in another callback, for example a
            // timer.
            timer = node->create_wall_timer(
                std::chrono::nanoseconds{0},
                [handle = std::move(handle), header = std::move(request_header),
                 request = std::move(request)]() {
                  beginner_tutorials::srv::PrintNewString::Response response;
                  response.new_string = request->s;
                  handle->send_response(*header, response);
                });
          });

  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
